...
def process_file(args):
    file, sr, session_length = args
    x, sr = sf.read(file)
    x = VAD(x, sr=sr, session_length=session_length)
    return [seg.T.astype(np.float32) for seg in x]


def musdb_preprocess(train_root_dir, test_root_dir, output_dir,
                     stem=None, sr=44100, session_length=6, max_samples_per_file=100):

    files_to_process = []
    for root, _, files in os.walk(train_root_dir):
        for file in files:
            if file.endswith(".wav") and (stem is None or file.split(".")[0] == stem):
    ...
    print(f"Found {len(files_to_process)} files (stem={stem}).")
    ...
    with Pool(n_workers) as pool:
        for segments in tqdm(pool.imap_unordered(process_file,
                                                 [(f, sr, session_length) for f in files_to_process]),
                             total=len(files_to_process), desc="Files"):
            for seg in segments:
                buffer.append(seg)
                if len(buffer) >= max_samples_per_file:
                    dataset_name = os.path.join(stem_dir, f"sample{cnt}.hdf5")
                    with h5py.File(dataset_name, 'w') as dataset:
                        dataset.create_dataset('data', data=np.asarray(buffer),
                                               dtype=np.float32,
...
if __name__ == "__main__":
    ...
    output_dir = "/tmp/prism/Apollo-Upgrade/hdf5_datas"

    # ⬇️ Change this to "vocals", "drums", "bass", "other", or "mixture"
    selected_stem = "vocals"

    musdb_preprocess(train_root_dir, test_root_dir, output_dir,
                     stem=selected_stem, sr=44100, session_length=6, max_samples_per_file=100)